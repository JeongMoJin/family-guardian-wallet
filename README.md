...
[![KFIP 2026 참가작](https://img.shields.io/badge/KFIP%202026-%EC%B0%B8%EA%B0%80%EC%9E%91-D97706?style=for-the-badge)](https://program.xrplkorea.org/)
[![XRPL Testnet](https://img.shields.io/badge/XRPL-Testnet-1F2937?style=for-the-badge)](https://xrpl.org/)
[![License: MIT](https://img.shields.io/badge/License-MIT-6B7280?style=for-the-badge)](#라이선스)

![가디언월렛](docs/images/01-title.png)
...
>
> 영상 흐름: Title → Problem (초고령사회·회색지대) → Solution (가족 2/3 승인) → DemoFlow (시연 모드 한 화면 분할 시뮬레이션) → Differentiator (왜 XRPL) → CTA. 결과 카드에는 실제 testnet `SignerListSet` tx hash (`D449E423...`) 가 노출된다.

## 핵심 흐름 (Remotion 시연 영상 발췌)
...
`submitAndWait` 응답이 `tesSUCCESS` 면 시니어 화면에 explorer 링크가 즉시 등장. 한 번의 흐름이 진짜 testnet 트랜잭션으로 끝까지 도는 것을 심사위원이 직접 클릭해 확인할 수 있다.
...
- `doc/Korea Financial Innovation Program 2026 신청서.docx` (원본 양식)
- `doc/KFIP_2026_신청서_가디언월렛.docx` (양식 100 % 보존 채움 사본)

배포 URL · 영상 URL · 연락처 확정 후 한 번 더 채울 수 있다:
...
PDF 변환 (Microsoft Word 가 깔린 Windows):

```powershell
$word = New-Object -ComObject Word.Application
$doc  = $word.Documents.Open('절대경로\doc\KFIP_2026_신청서_가디언월렛.docx')
$doc.SaveAs([ref] '절대경로\doc\KFIP_2026_신청서_가디언월렛.pdf', [ref] 17)
$doc.Close(); $word.Quit()
```
